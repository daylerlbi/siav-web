...
export default function AdminProyectos() {
   ...
   const phaseOptions = [
      ...
      ...Array.from({ length: 8 }, (_, i) => ({ id: i + 1, value: `Fase ${i + 1}` }))
   ]
   ...
   useEffect(() => {
      ...
      // Estado de revisión
      if (filters.estadoRevision !== "ALL") {
         filtered = filtered.filter(p => {
            if (filters.estadoRevision === null || filters.estadoRevision === "SIN_REVISAR") {
               return p.estadoRevision === null || p.estadoRevision === "SIN_REVISAR"
            }
            return p.estadoRevision === filters.estadoRevision
         })
      }
   ...
   const removeFilter = (filterType) => {
      ...
      else if (filterType === 'estadoRevision') setFilters(f => ({ ...f, estadoRevision: "ALL" }))
   ...
   const estadoRevisionOptions = [
      { id: "ALL", value: "Todos los estados" },
      { id: "SIN_REVISAR", value: "Fases en edición" },
      { id: "EN_REVISION", value: "Fases por revisar" },
      { id: "RECHAZADA", value: "Fases Rechazadas" },
      { id: "ACEPTADA", value: "Fases Aprobadas" }
   ]
      ...
      <main className="flex flex-col gap-4 h-full">
         ...
         <section className="flex justify-between items-center gap-4 w-full">
            ...
            <CustomSelect
               ...
               action={opt => setFilters(f => ({
                  ...f,
                  estadoRevision: opt.id
               }))}
               options={estadoRevisionOptions}
               defaultValue={estadoRevisionOptions.find(e => e.id === filters.estadoRevision) || estadoRevisionOptions[0]}
               label="Estado actual de la fase"
            />
            <CustomSelect
               className="flex justify-between items-center rounded-md border w-full p-2"
               action={opt => setFilters(f => ({
            ...
            <section className="flex flex-wrap gap-2 mt-2 text-sm">
               ...
               {filters.estadoRevision !== "ALL" && (
                  <div className="bg-rojo-claro text-rojo-institucional border-rojo-institucional border flex items-center gap-2 px-3 py-1 rounded-md">
                     <span>
                        <b>Estado actual:</b> {
                           estadoRevisionOptions.find(e => e.id === filters.estadoRevision)?.value
                        }
                     </span>
                     <button
                        onClick={() => removeFilter('estadoRevision')}
                        className="hover:bg-rojo-institucional hover:text-white duration-150 rounded-full p-1"
                        aria-label="Eliminar filtro de estado actual"
                     >
                        <X size={16} />
                     </button>
                  </div>
