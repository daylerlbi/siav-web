  ...
  useEffect(() => {
    if (ruta === 'grupos') {
      if (usuario.codigo !== '') {
        ...
        fetch(
          `${backendUrl}/oracle/materias-matriculadas/alumno-carrera?codCarrera=${usuario.codigo.slice(0, 3)}&codAlumno=${usuario.codigo.slice(3)}`
        )
      ...
      fetch(`${backendUrl}/oracle/materias/tecnologia`)
        .then((response) => response.json())
        .then((data) => {
          setMaterias(data)
        })
