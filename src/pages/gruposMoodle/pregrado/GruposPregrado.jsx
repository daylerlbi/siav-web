...
const GruposPregrado = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${backendUrl}/oracle/grupos/carrera?codCarrera=215`).then(
        (response) => response.json()
      ),
      fetch(`${backendUrl}/oracle/materias/tecnologia`).then((response) =>
        response.json()
      ),
      fetch(`${backendUrl}/oracle/profesores`).then((response) =>
        response.json()
      )
    ])
  ...
  const obtenerMateriaOracle = async (codCarrera, codMateria) => {
    const response = await fetch(
      `${backendUrl}/oracle/materias/buscar?codCarrera=${codCarrera}&codMateria=${codMateria}`
    )
  ...
  const traerInformacionGrupoOracle = async (codCarrera, codMateria, grupo) => {
    const response = await fetch(
      `${backendUrl}/oracle/grupos/grupo?codCarrera=${codCarrera}&codMateria=${codMateria}&grupo=${grupo}`
    )
    if (!response.ok) {
  ...
  const obtenerProfesorOracle = async (codProfesor) => {
    const response = await fetch(
      `${backendUrl}/oracle/profesores/buscar/codigo?codProfesor=${codProfesor}`
    )
    if (!response.ok) {
  ...
  const obtenerEstudiantesMatriculados = async (
    ...
    try {
      const response = await fetch(
        `${backendUrl}/oracle/materias-matriculadas/filtrar?codCarMat=${codCarMat}&codMatMat=${codMatMat}&grupo=${grupo}`
      )
      if (!response.ok) {
  ...
  const obtenerEstudianteOracle = async (codigoCompleto) => {
    try {
      const response = await fetch(
        `${backendUrl}/oracle/estudiantes/buscar/codigo?codigo=${codigoCompleto}`
      )
      if (!response.ok) {
