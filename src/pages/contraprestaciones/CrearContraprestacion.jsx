...
const CrearContraprestacion = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl}/contraprestaciones/tipos`)
      ...
      .catch((err) => {
        showAlerta(err.message, 'error', 'Error al cargar tipos')
      })
    ...
    fetch(`${backendUrl}/estudiantes/listar/estado/1`)
      ...
      .catch((err) => {
        showAlerta(err.message, 'error', 'Error al cargar estudiantes')
      })
  }, [backendUrl])
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data && data.message) {
          ...
          errorMessage =
            'No se encontró el estudiante o tipo de contraprestación especificado'
        } else if (response.status === 409) {
          errorMessage =
            'Ya existe una contraprestación para este estudiante en el semestre actual'
        }
      ...
      showAlerta(successMessage, 'success', 'Contraprestación creada')

      limpiarCampos()

      // Navegar después de 2 segundos para dar tiempo a ver el mensaje
      setTimeout(() => {
        navigate('/matricula/contraprestaciones')
      }, 2000)
    } catch (err) {
      // Mostrar el mensaje de error exactamente como viene del backend
      showAlerta(err.message, 'error', 'Error al crear contraprestación')
    } finally {
      setLoading(false)
    }
  }
