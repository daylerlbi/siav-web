...
import EliminarInformeModal from "../../components/admin/EliminarInformeModal"
...
export default function ListadoInformes() {
   const { listInformesDocente, createColoquio, editColoquio, listEntregasColoquio, listDocumentosEntregasColoquio, deleteInforme } = useInformes()
   ...
   const handleEliminarInforme = async (informe) => {
   ...
   return (
      <>
         <main className="flex flex-col gap-4 h-full">
               ...
               <table className="flex flex-col rounded-md border w-full">
                  ...
                  <tbody className="text-sm">
                     {loading ? (
                        ...
                        <tr>
                           <td colSpan={6} className="text-center p-8 text-gris-institucional">
                              ...
                              >
                                 ...
                                 {openActions[inf.id] && (
                                    <div className="bg-white whitespace-nowrap border rounded-md absolute z-50 top-[115%] right-0 select-animation flex flex-col">
                                       ...
                                       <button
                                          className="hover:bg-red-50 duration-150 p-2 text-left flex items-center gap-2 w-full"
                                          onClick={() => {
                                             setOpenActions(prev => ({ ...prev, [inf.id]: false }))
                                             setInformeAEliminar(inf)
                                          }}
                                       >
                                          <Trash2 size={16} className="text-red-600" />
                                          <span className="text-red-600">Eliminar informe</span>
                                       </button>
                                    </div>
                                 )}
                              </td>
            ...
            {
               informeAEliminar &&
               <EliminarInformeModal
                  informe={informeAEliminar}
                  isOpen={setInformeAEliminar}
