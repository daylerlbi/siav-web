...
const Aplazamientos = () => {
  ...
  const fetchSolicitudes = () => {
    ...
    fetch(`${backendUrl}/api/solicitudes/aplazamiento`)
      ...
      .catch(() => showAlerta('Error al cargar las solicitudes de aplazamiento', 'error', 'Error de conexión'))
      .finally(() => setCargandoAplazamientos(false))
  }
  ...
  const crearSolicitudAplazamiento = async (e) => {
    ...
    setCreando(true)
    try {
      const r = await fetch(`${backendUrl}/api/solicitudes/aplazamiento/crear`, {
    ...
    } catch (error) { showAlerta(error.message, 'error', 'Error al crear la solicitud') }
  ...
  const actualizarSolicitudAplazamiento = async (e) => {
    ...
    setEditando(true)
    try {
      const r = await fetch(`${backendUrl}/api/solicitudes/aplazamiento/actualizar/${editSolicitudId}`, {
      ...
      showAlerta(data.mensaje || data.message || 'Solicitud actualizada con éxito', 'success', 'Solicitud actualizada')
    } catch (error) { showAlerta(error.message, 'error', 'Error al actualizar la solicitud') }
    finally { setEditando(false) }
  }

  const submitAprobarAplazamiento = async () => {
    if (!selectedFile) { setFileError('Debe seleccionar un archivo para aprobar el aplazamiento'); return }
    if (!currentSolicitud?.Id) { showAlerta('No se ha seleccionado una solicitud válida', 'error', 'Error de validación'); return }
    ...
    try {
      const r = await fetch(`${backendUrl}/api/solicitudes/aplazamiento/aprobar/${currentSolicitud.Id}`, { method: 'POST', body: formData, headers: { 'X-Usuario': nombreUsuario } })
      const data = await r.json().catch(() => ({ mensaje: r.ok ? 'Aprobado con éxito' : 'Error al aprobar' }))
      if (!r.ok) throw new Error(data.mensaje || 'Error al aprobar el aplazamiento')
      fetchSolicitudes(); setIsApproveModalOpen(false); setSelectedFile(null); setFileName('')
      showAlerta(data.message || data.mensaje || 'Solicitud aprobada con éxito', 'success', 'Solicitud aprobada')
  ...
  return (
    <div className='w-full p-4'>
      <div className='w-full flex items-center justify-between mb-8'>
        <p className='text-center text-titulos flex-1'>Aplazamiento de Semestre</p>
        <Boton onClick={handleOpenRegisterModal} color='danger'>Registrar Aplazamiento</Boton>
      </div>

      <TablaEstados informacion={informacion} columnas={columnas} filtros={filtros} accionesPorEstado={accionesPorEstado} campoEstado='estaAprobado' elementosPorPagina={10} cargandoContenido={cargandoAplazamientos} />

      <Modal isOpen={isApproveModalOpen} onOpenChange={(open) => { if (!approving) setIsApproveModalOpen(open) }} cabecera='' size='xl'
        cuerpo={
          <div>
            <div className='flex flex-col gap-1 text-center mb-6'><p className='text-2xl font-semibold text-titulos'>Aprobar Solicitud de Aplazamiento</p></div>
            {currentSolicitud && <p>¿Está seguro que desea aprobar la solicitud de aplazamiento de semestre para el estudiante {currentSolicitud.Estudiante}?</p>}
            <p className='mt-2 mb-1'>Esta acción aprobará el aplazamiento del semestre {currentSolicitud?.['Semestre Aplazado']}, le desmatriculará todas las materias que tiene en curso y cambiará el estado del estudiante a Inactivo.</p>
        ...
        footer={<div className='flex justify-end w-full'><Boton color='success' onClick={submitAprobarAplazamiento} disabled={approving || !selectedFile} startContent={approving ? null : <CheckCircle size={18} />}>{approving ? 'Aprobando...' : 'Aprobar'}</Boton></div>}
      />

      <Modal isOpen={isRegisterModalOpen} onOpenChange={(open) => { setIsRegisterModalOpen(open); if (!open) limpiarFormularioRegistro() }} cabecera='Registrar Aplazamiento de Semestre' size='xl'
        cuerpo={
          <Form className='flex flex-col gap-4' onSubmit={crearSolicitudAplazamiento}>
            {!isEstudiante && (
            ...
            <div className='w-full'>
              ...
              <Textarea classNames={{ inputWrapper: 'border border-gris-institucional rounded-[15px] w-full' }} placeholder='Ingrese el motivo detallado para el aplazamiento del semestre' value={descripcion} onChange={(e) => setDescripcion(e.target.value)} minRows={4} isRequired />
            </div>
            <div className='w-full flex justify-end mb-[-20px]'><Boton type='submit' disabled={creando}>{creando ? 'Creando...' : 'Registrar Aplazamiento'}</Boton></div>
          </Form>
        }
      />

      <Modal isOpen={isEditModalOpen} onOpenChange={(open) => { setIsEditModalOpen(open); if (!open) limpiarFormularioEdicion() }} cabecera='Editar Solicitud de Aplazamiento' size='xl'
        cuerpo={
          <Form className='flex flex-col gap-4' onSubmit={actualizarSolicitudAplazamiento}>
            {!isEstudiante && (
              <div className='w-full py-4'>
            ...
            <div className='w-full'>
              <p className='font-medium mb-2'>Motivo de la Solicitud</p>
              <Textarea classNames={{ inputWrapper: 'border border-gris-institucional rounded-[15px] w-full' }} placeholder='Ingrese el motivo detallado para el aplazamiento del semestre' value={editDescripcion} onChange={(e) => setEditDescripcion(e.target.value)} minRows={4} isRequired />
            </div>
            <div className='w-full flex justify-end mb-[-20px]'><Boton type='submit' color='success' disabled={editando}>{editando ? 'Actualizando...' : 'Actualizar Aplazamiento'}</Boton></div>
          </Form>
        }
      />
