...
const VerAplazamiento = () => {
  ...
  const fetchAplazamientoData = () => {
    setLoading(true)
    fetch(`${backendUrl}/solicitudes/solicitud/${id}`)
      .then((response) => {
        if (!response.ok) {
      ...
      .catch((err) => {
        ...
        setLoading(false)
        showAlerta(
          'No se pudo cargar la información del aplazamiento',
          'error',
          'Error de conexión'
        )
      })
  ...
  const handleApproveAplazamiento = async () => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/solicitudes/aplazamiento/aprobar/${id}`,
        {
          method: 'POST',
          body: formData,
          headers: {
      ...
      showAlerta(
        data.mensaje || 'Aplazamiento aprobado con éxito',
        'success',
        'Solicitud aprobada'
      )
    } catch (err) {
  ...
  const handleDownloadReport = async () => {
    if (!solicitud.soporte || !solicitud.soporte.id) {
      showAlerta(
        'No hay informe disponible para descargar',
        'error',
        'Error de descarga'
      )
      return
    ...
    try {
      ...
      showAlerta(
        `Error al descargar el informe: ${error.message}`,
        'error',
        'Error de descarga'
      )
    } finally {
  ...
  if (error) {
    return (
      <div className='flex flex-col items-center justify-center h-screen'>
        ...
        <Boton
          onClick={() => navigate('/aplazamientos')}
    ...
    <div className='flex flex-col w-full items-center p-4'>
      <div className='w-full flex flex-row justify-between mb-6'>
        <button
          ...
          onClick={() => navigate('/matricula/aplazamiento')}
        >
          <ArrowLeft />
        </button>
        <p className='text-center text-titulos'>
          Detalles de Solicitud de Aplazamiento
        </p>
      ...
      {solicitud.estaAprobado && solicitud.soporte && (
        <>
          <p className='text-normal mt-8'>
            Informe de Aplazamiento de Semestre
          </p>
          <Divider className='mb-4' />
          <div className='w-full px-4'>
              ...
              <div className='flex-grow'>
                <div className='flex items-center'>
                  ...
                  <p className='font-medium'>
                    {solicitud.soporte.nombre_archivo}
                  </p>
                </div>
                <p className='text-sm text-gray-500 mt-1'>
                  Tipo: {solicitud.soporte.mime_type} | Tamaño:{' '}
                  {solicitud.soporte.peso}
                </p>
                <p className='text-sm text-gray-500'>
                  Subido el {formatDate(solicitud.soporte.fecha_subida)}
                </p>
              </div>
              <div>
                <Boton
        ...
        cuerpo={
          <div>
            ...
            <p>
              ...
              semestre para el estudiante {solicitud.estudianteNombre}?
            </p>
            <p className='mt-2 mb-1'>
              Esta acción aprobará el aplazamiento del semestre{' '}
              {solicitud.semestre}, le desmatriculará todas las materias que
              tiene en curso y cambiará el estado del estudiante a Inactivo.
            </p>
