  ...
  const fetchCancelacionData = () => {
    ...
    fetch(`${backendUrl}/solicitudes/solicitud/${id}`)
      ...
      .catch((err) => {
        ...
        setLoading(false)
        showAlerta(
          'No se pudo cargar la información de la cancelación',
          'error',
          'Error de conexión'
        )
      })
  ...
  const handleApproveCancelacion = async () => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/solicitudes/cancelacion/aprobar/${id}`,
        {
          method: 'POST',
          body: formData,
          headers: {
      ...
      showAlerta(
        data.mensaje || 'Cancelación aprobada con éxito',
        'success',
        'Solicitud aprobada'
      )
    } catch (err) {
    ...
    <div className='flex flex-col w-full items-center p-4'>
      <div className='w-full flex flex-row justify-between mb-6'>
        <button
          ...
          onClick={() => navigate('/matricula/cancelaciones')}
        >
          <ArrowLeft />
        </button>
        ...
        cuerpo={
          <div>
            ...
            <p>
              ...
              materia <strong>{solicitud.grupoNombre?.split(' - ')[0]}</strong>{' '}
              para el estudiante <strong>{solicitud.estudianteNombre}</strong>?
            </p>
            <p className='mt-2 mb-1'>
              Esta acción cancelará la materia{' '}
              {solicitud.grupoNombre?.split(' - ')[0]} del grupo{' '}
              {solicitud.grupoCodigo} para el semestre {solicitud.semestre}.
            </p>
