...
const CrearEstudiante = () => {
  ...
  const minDateIngreso = today(getLocalTimeZone()).subtract({ years: 10 }) // Fecha mínima para fecha de ingreso (pueden ingresar desde hace 10 años máximo)
  const maxDateIngreso = today(getLocalTimeZone()).add({ years: 1 }) // Fecha máxima para fecha de ingreso (hasta 1 año en el futuro)
  ...
  if (codigo && !/^247/.test(codigo)) {
    codigoErrors.push('Debe empezar con 247')
  }
  ...
  useEffect(() => {
    fetch(`${backendUrl}/programas/listar`)
      ...
      .then((data) => {
        ...
        const programasPosgrado = data.filter(
          (programa) => programa.esPosgrado === true
        )
        setProgramas(programasPosgrado)
      })
  }, [])
  ...
  const buscarEstudiante = async () => {
    if (codigoErrors.length === 0) {
      fetch(`${backendUrl}/oracle/estudiantes/buscar/codigo?codigo=${codigo}`)
        .then((response) => response.json())
        .then((data) => {
          if (data.length > 0) {
  ...
  const crearEstudiante = async () => {
    ...
    // Manejar fecha de ingreso - si no se especifica, usar fecha de hoy
    const formattedFechaIngreso = fechaIngreso
      ? fechaIngreso.toDate(getLocalTimeZone()).toISOString().split('T')[0]
      : new Date().toISOString().split('T')[0] // Fecha de hoy si no se especifica
          ...
          <div className='w-full flex flex-row mb-4'>
            ...
            <div className='w-1/2'>
              <DatePicker
                ref={fechaIngresoRef}
                classNames={{
                  label: `w-2/6 h-[40px] flex items-center pl-2`,
                  base: 'flex items-start',
                  inputWrapper:
                    'border border-gris-institucional rounded-[15px] w-1/2 max-h-[40px]'
                }}
                ...
                minValue={minDateIngreso}
                maxValue={maxDateIngreso}
                value={fechaIngreso || undefined}
                onChange={(value) => setFechaIngreso(value)}
                placeholder='Si no se especifica, se usará la fecha de hoy'
                description='Opcional: Si no se especifica, se usará la fecha actual'
              />
            </div>
          </div>
        </div>
