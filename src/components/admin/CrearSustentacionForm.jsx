...
export default function CrearSustentacionForm({ isOpen, proyecto, sustentacion }) {
   const { createSustentacion, obtenerDocentes, asignarJurados, actualizarFase, updateSustentacion } = useAdmin()
   ...
   useEffect(() => {
      setFormData(f => ({
         ...f,
         idProyecto: proyecto?.id,
         tipoSustentacion: sustentacion?.tipoSustentacion || (proyecto?.estadoActual === 3
            ? "ANTEPROYECTO"
            : proyecto?.estadoActual === 7
               ? "TESIS"
               : ""),
         fecha: sustentacion?.fecha || "",
         hora: sustentacion?.hora || "",
         horaFin: sustentacion?.horaFin || "",
         lugar: sustentacion?.lugar || "",
         descripcion: sustentacion?.descripcion || "",
         sustentacionExterna: sustentacion?.sustentacionExterna || false,
         docenteId: sustentacion?.tipoSustentacion === "ANTEPROYECTO"
            ? (sustentacion?.evaluadores?.map(e => String(e.idUsuario)) || [])
            : [],
         juradosTesis: sustentacion?.tipoSustentacion === "TESIS"
            ? (sustentacion?.evaluadores?.map(e => ({
               id: String(e.idUsuario),
               externo: !!e.juradoExterno
            })) || [])
            : []
      }))
   ...
   const handleSubmit = async (e) => {
      ...
      try {
         ...
         console.log("Sustentación actualizada", sustentacion)
         if (sustentacion) {
            const res = await updateSustentacion({ id: sustentacion.id, body: formData })
         } else {
            ...
            <form onSubmit={handleSubmit} className="space-y-2 mt-4 text-sm">
               ...
               <div className="flex items-stretch gap-2 pt-4">
                  ...
                  <Boton type="submit" variant="borderwhite">
                     {sustentacion ? "Editar sustentación" : "Crear sustentación"}
                  </Boton>
               </div>
            </form>
         </section>
