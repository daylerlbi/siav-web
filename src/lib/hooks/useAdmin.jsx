...
export default function useAdmin() {
   ...
   const updateSustentacion = async ({ id, body }) => {
      try {
         let token = accessToken
         if (!accessToken) token = googleToken
         const data = await editarSustentacion({ body: body, id: id, accessToken: token })
         return data
      } catch (error) {
         setIsError(true)
         setError(error)
   ...
   const completeSustentacion = async (id) => {
      try {
         let token = accessToken
         if (!accessToken) token = googleToken
         const data = await completarSustentacion({ id: id, accessToken: token })
         return data
      } catch (error) {
         setIsError(true)
         setError(error)
   ...
   return { obtenerProyectos, listProgramas, completeSustentacion, updateSustentacion, deleteGrupo, editGrupo, createLinea, editLinea, deleteLinea, deleteProject, createGrupo, importProyecto, importDocumentos, obtenerGrupos, actualizarFase, createSustentacion, obtenerDocentes, asignarJurados, asignarDirectores, guardarComentariosJurados }
}